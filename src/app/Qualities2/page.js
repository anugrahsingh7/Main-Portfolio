  ...
  return (
    <div id="strengths" className="min-h-screen bg-black text-white px-4 sm:px-6 py-8 sm:py-12">
      <motion.h1
        ...
        className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-center mb-12 sm:mb-20 bg-clip-text text-transparent bg-gradient-to-r from-red-500 via-rose-500 to-pink-500"
      >
        My Core Strengths
      </motion.h1>

      <motion.div 
        className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 md:gap-12 px-4 sm:px-0"
      ...
      >
        {personalitySkills.map((skill, index) => (
          <motion.div
            ...
            className="bg-gradient-to-tr from-zinc-800/60 to-zinc-900/80 border border-red-500/20 p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-lg hover:shadow-red-500/30 hover:scale-105 transition-transform duration-300 will-change-transform"
            style={{ transform: 'translateZ(0)' }}
          >
            <div className="flex items-center space-x-3 sm:space-x-4 mb-3 sm:mb-4">
              {skill.icon}
              <h2 className="text-xl sm:text-2xl font-bold text-red-400">{skill.title}</h2>
            </div>
            <p className="text-zinc-300 text-sm sm:text-base leading-relaxed">{skill.description}</p>
          </motion.div>
        ))}
      </motion.div>
